...
**Per-user topic filters (migration 0068, `internal/kbfilters`):** the shell's chip row — "Alle" / "Favoriten" / the caller's own categories with a "+". Three tables, all personal state and never an access grant: `kb_favourites` (row existence IS the star; there is no `is_favourite = false`), `kb_user_categories` (unique on `(user_id, LOWER(name))` — two chips reading "Studium" and "studium" are indistinguishable in a filter row), and `kb_user_category_links`. Deliberately separate from `kb_categories`/`kb_category_links`: those are one system-admin-curated taxonomy over PUBLIC KBs (the catalog's filter tabs), these belong to one user and may point at any topic that user can see. `kb_user_category_links.user_id` is redundant next to `category_id` on purpose — the composite FK `(category_id, user_id) REFERENCES kb_user_categories (id, user_id)` makes "assigned somebody else's category" **unrepresentable in the database** rather than merely unreachable through the handler; `kbfilters` maps that 23503 to a 404. Eight routes: `PUT`/`DELETE /api/kb/{id}/favourite`, CRUD on `/api/kb-user-categories[/{catId}]`, `PUT`/`DELETE /api/kb/{id}/user-categories/{catId}`. The `{id}` half sits on `kbViewChain` (not because starring is a privilege, but so the route cannot confirm the existence of a KB the caller may not see); the rest is authentication-only. `isFavourite` + `userCategoryIds` ride along on `GET /api/kb`, `/api/kb/global`, `/api/kb/{id}` and `/api/kb/catalog` as joins — the chip row filters one payload client-side, so a per-card request would be an N+1 on every view.
...
| 0068 | `kb_favourites`, `kb_user_categories`, `kb_user_category_links` (per-user topic filters) |
